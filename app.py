...
from meet_parser import (
    parse_pdf_text, extract_pdf_text, parse_hytek_text,
    build_event_lookup, simulate_placement, optimize_lineup,
    seconds_to_display, parse_time_to_seconds
)
...
@app.route("/meet/parse", methods=["POST"])
def meet_parse():
    """
    Parse a meet results file (PDF, HY-TEK .hy3/.cl2, or ZIP containing these).
    Upload as multipart form: field name = 'file'

    Returns all parsed swimmer results with times, places, gender, event.
    """
    if 'file' not in request.files:
        return jsonify({"error": "No file uploaded. Use multipart form with field 'file'."}), 400
            ...
            with zipfile.ZipFile(io.BytesIO(file_bytes)) as zf:
                for name in zf.namelist():
                    ...
                    if name_lower.endswith('.pdf'):
                        ...
                        results.extend(parse_pdf_text(text))
                    elif name_lower.endswith(('.hy3', '.cl2', '.txt')):
                        text = data.decode('utf-8', errors='ignore')
                        results.extend(parse_hytek_text(text))

        elif filename.endswith('.pdf'):
            text = extract_pdf_text(file_bytes)
            results = parse_pdf_text(text)

        elif filename.endswith(('.hy3', '.cl2', '.txt')):
            text = file_bytes.decode('utf-8', errors='ignore')
            results = parse_hytek_text(text)

        else:
            return jsonify({"error": f"Unsupported file type: {filename}. Use PDF, .hy3, .cl2, .txt, or .zip"}), 400

        if not results:
            return jsonify({"error": "No results could be parsed from this file. Check that it is a valid meet results file."}), 422

        # Build summary
        events = list(set(f"{r['gender']} {r['event']}" for r in results))
        events.sort()
...
@app.route("/meet/analyze", methods=["POST"])
def meet_analyze():
    """
    Analyze where a set of swimmers would place in a parsed meet field.

    Body (JSON):
    {
        "results": [...],          // output from /meet/parse
        "swimmers": [
            {
                "name": "Jacob Smith",
                "gender": "Men",
                "events": [
                    {"event": "100 Freestyle", "predicted_seconds": 48.5},
                    {"event": "200 Butterfly", "predicted_seconds": 115.2}
                ]
            }
        ]
    }
    ...
    swimmers = body.get("swimmers", [])

    if not results:
        return jsonify({"error": "No meet results provided. Run /meet/parse first."}), 400
    if not swimmers:
        return jsonify({"error": "No swimmers provided."}), 400

    lookup = build_event_lookup(results)

    analysis = []
    for swimmer in swimmers:
        name = swimmer.get("name", "Unknown")
        gender = swimmer.get("gender", "Women")
        ...
        for entry in events:
            ...
            pred_seconds = entry.get("predicted_seconds")

            if not event or pred_seconds is None:
                continue

            placement = simulate_placement(pred_seconds, gender, event, lookup)
            total_projected_points += placement["projected_points"]

            swimmer_results.append({
                "event": event,
                "predicted_time": seconds_to_display(pred_seconds),
                "predicted_seconds": pred_seconds,
                **placement,
            })

        # Sort events by projected points descending
        swimmer_results.sort(key=lambda x: x["projected_points"], reverse=True)

        analysis.append({
            "name": name,
            "gender": gender,
            "total_projected_points": total_projected_points,
            "events": swimmer_results,
        })

    # Sort swimmers by total projected points
    analysis.sort(key=lambda x: x["total_projected_points"], reverse=True)

    return jsonify({
        "success": True,
        "team_total_projected_points": sum(s["total_projected_points"] for s in analysis),
        "swimmers": analysis,
    })


@app.route("/meet/optimize", methods=["POST"])
def meet_optimize():
    """
    Find the optimal event assignments to maximize team points.
    ...
    Body (JSON):
    {
        ...
        "max_events_per_swimmer": 3,      // optional, default 3
        "max_entries_per_event": null     // optional, null = unlimited
    }
    """
    body = request.get_json(silent=True) or {}
    results = body.get("results", [])
    swimmers = body.get("swimmers", [])
    max_events = body.get("max_events_per_swimmer", 3)
    max_entries = body.get("max_entries_per_event", None)
    ...
    lookup = build_event_lookup(results)
    optimization = optimize_lineup(swimmers, lookup, max_events, max_entries)
...
@app.route("/meet/events", methods=["POST"])
def meet_events():
    """
    Get a summary of all events and field stats from parsed results.
    Useful for building event bank / event picker UI.
    ...
    for gender in lookup:
        for event in lookup[gender]:
            finals = lookup[gender][event]['finals']
            prelims = lookup[gender][event]['prelims']
            prelim_times = sorted([r['prelim_time'] for r in prelims if r['prelim_time']])

            winner = finals[0] if finals else None
            ...
            events_summary.append({
                "gender": gender,
                "event": event,
                "total_prelim_entries": len(prelim_times),
                "total_finalists": len(finals),
                "winning_time": seconds_to_display(winner['finals_time']) if winner else '--',
                "winning_time_seconds": winner['finals_time'] if winner else None,
                "cutline_a_final": seconds_to_display(cutline_a),
                "cutline_a_final_seconds": cutline_a,
                "cutline_b_final": seconds_to_display(cutline_b),
                "cutline_b_final_seconds": cutline_b,
            })

    # Sort: Women first, then by event order
    EVENT_ORDER = ['50 Freestyle', '100 Freestyle', '200 Freestyle', '500 Freestyle',
                   '1000 Freestyle', '1650 Freestyle', '100 Backstroke', '200 Backstroke',
                   '100 Breaststroke', '200 Breaststroke', '100 Butterfly', '200 Butterfly',
                   '200 IM', '400 IM']

    def sort_key(e):
        gender_order = 0 if e['gender'] == 'Women' else 1
        try:
            event_order = EVENT_ORDER.index(e['event'])
        except ValueError:
            event_order = 99
        return (gender_order, event_order)

    events_summary.sort(key=sort_key)
